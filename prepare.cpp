...
double calculateAttenuation(double distance, double intensity, 
    ...
    if (intersectScene(shadowRay, shadowHit)) {
        if (shadowHit.t < distance - 0.001) {
            ...
            if (shadowHit.materialType == 3) {
                baseAttenuation *= 0.5; // 半透明材质衰减一半
            } else {
                return 0.0; // 完全不透明材质，完全遮挡
            }
        }
    }
    ...
    if (lightRadius > 0.0) {
        ...
        // 构建光源平面的坐标系
        double lightDirNormalized[3] = {lightDir[0], lightDir[1], lightDir[2]};
        normalize(lightDirNormalized);
        
        double tangent[3], bitangent[3];
        
        // 选择与光线方向不垂直的参考向量
        double reference[3] = {0.0, 1.0, 0.0};
        if (fabs(dot(lightDirNormalized, reference)) > 0.9) {
            reference[0] = 1.0; reference[1] = 0.0; reference[2] = 0.0;
        }
        
        // 计算切线向量
        cross(lightDirNormalized, reference, tangent);
        normalize(tangent);
        
        // 计算副切线向量
        cross(lightDirNormalized, tangent, bitangent);
        normalize(bitangent);
        
        // 简单采样：统计可见比例
        for (int i = 0; i < numSamples; i++) {
            // 在圆盘上随机采样
            double r = lightRadius * sqrt(rand() / (double)RAND_MAX);
        ...
        visibleRatio /= numSamples; // 计算可见比例
        
        // 应用软阴影：可见比例越接近1，阴影越柔和
        double softShadowFactor = visibleRatio;
        
        // 如果硬阴影已经判断被遮挡，但软阴影有部分可见，给予一些亮度
        if (inShadow && visibleRatio > 0.0) {
            baseAttenuation = intensity * 0.3 * visibleRatio / (1.0 + 0.01 * distance * distance);
        } else {
            baseAttenuation *= softShadowFactor;
        }
    }
...
void processInput() {
    ...
    pointLights[0].position[0]=camera.x;
    pointLights[0].position[1]=camera.y;
    pointLights[0].position[2]=camera.z;
...
int main() {
    ...
    // 添加点光源
    addPointLight(10, 100, 100, 1.0, 1.0, 1.0, 1, 0);
    
    // 加载模型（每个模型是一个独立物体）
    cout << ProcessModelWithTexture("death_box/death_box.obj", L"death_box/death_box.png") << endl;
    
    // 可以加载更多模型
    // cout << ProcessModelWithTexture("another_model.obj", L"another_texture.png") << endl;
    
    cout << "总三角形数量: " << triangleCount << endl;
    cout << "物体数量: " << blasList.size() << endl;
    ...
    while(true) {
        ...
        // 动态更新：可以在这里更新动态物体的BLAS
        // if(someObjectMoved) {
        //     updateBLAS(movingObject);
        //     rebuildTLAS(); // 或者增量更新TLAS
        // }
